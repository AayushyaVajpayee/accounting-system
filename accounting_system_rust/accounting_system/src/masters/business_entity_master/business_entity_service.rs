...
#[allow(dead_code)]
pub fn get_business_entity_master_service(arc: Arc<Pool>, address_service: Arc<dyn AddressService>) -> Arc<dyn BusinessEntityService> {
...
impl BusinessEntityService for BusinessEntityServiceImpl {
    async fn get_business_entity_by_id(&self, id: &Uuid, tenant_id: &Uuid) -> Result<Option<Arc<BusinessEntityDto>>, BusinessEntityServiceError> {
                            ...
                            async {
                                ...
                                let business_entity_dto = if let Some(entity) = business_entity_master {
                                    let address = if let Some(addr_id) = entity.entity_type.get_address_id() {
                                        self.address_service.get_address_by_id(*tenant_id, addr_id).await
