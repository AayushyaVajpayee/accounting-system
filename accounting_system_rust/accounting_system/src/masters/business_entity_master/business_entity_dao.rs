...
use crate::masters::business_entity_master::business_entity_models::{
    BusinessEntityMaster, BusinessEntityName, BusinessEntityType, CreateBusinessEntityRequest,
    Email, PhoneNumber,
};
...
pub trait BusinessEntityDao: Send + Sync {
    ...
    async fn get_business_entity(
        &self,
        id: &Uuid,
        tenant_id: &Uuid,
    ) -> Result<Option<BusinessEntityMaster>, DaoError>;
    async fn is_business_entity_exist(&self, id: &Uuid, tenant_id: &Uuid)
    ...
    fn try_from(row: Row) -> Result<Self, Self::Error> {
        ...
        let e_type: BusinessEntityType = if eligible_supplier {
            ...
            BusinessEntityType::Other {
                name: BusinessEntityName::new(row.get(next_ind + 1))?,
                email: row
                    .get::<usize, Option<&str>>(next_ind + 2)
                    .map(|a| Email::new(a))
                    .transpose()?,
                phone: PhoneNumber::new(row.get(next_ind + 3))?,
                address_id: row.get(next_ind + 4),
                gstin: row
                    .get::<usize, Option<&str>>(next_ind + 5)
                    .map(|a| GstinNo::new(a))
                    .transpose()
                    .context("error during db row conversion")?,
            }
...
impl BusinessEntityDao for BusinessEntityDaoImpl {
    ...
    async fn get_business_entity(
        ...
        let en: Option<BusinessEntityMaster> = self
            .postgres_client
            .get()
            .await?
    ...
    async fn is_business_entity_exist(
        &self,
        id: &Uuid,
        tenant_id: &Uuid,
    ) -> Result<bool, DaoError> {
        let row = self
            ...
            .await?
            .query_one(
                "select exists (select 1 from business_entity where id=$1 and tenant_id=$2)",
                &[id, tenant_id],
            )
            .await?;
...
mod tests {
    ...
    use crate::masters::business_entity_master::business_entity_dao::{
        BusinessEntityDao, BusinessEntityDaoImpl,
    };
    use crate::masters::business_entity_master::business_entity_models::tests::{
        a_create_business_entity_request, SEED_BUSINESS_ENTITY_ID2,
    };
    ...
    async fn test_is_business_entity_exist() {
        ...
        .await;
        let exist = dao
            .is_business_entity_exist(&SEED_BUSINESS_ENTITY_ID2, &SEED_TENANT_ID)
            .await
            .unwrap();
        let not_exist = dao
            .is_business_entity_exist(&Uuid::now_v7(), &SEED_TENANT_ID)
            .await
            .unwrap();
        assert!(exist);
        assert!(!not_exist);
