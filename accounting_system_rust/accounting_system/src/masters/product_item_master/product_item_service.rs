...
impl ProductItemService for ProductItemServiceImpl {
    ...
    async fn get_products(&self, product_ids: Vec<Uuid>, tenant_id: Uuid)
        ...
        let uncached_product_ids: Vec<Uuid> = uncached_product_ids.into_iter().map(|a| a.0).collect();
        let db_products = self.dao.get_products(&uncached_product_ids, tenant_id).await?;
        ...
        for product in db_products {
            ...
            self.cache_by_tenant_id_and_product_id.insert((tenant_id, product_id), product_arc.clone()).await;
