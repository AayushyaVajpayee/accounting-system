...
async fn get_product_item(
    ...
    let ap = data.get_product(
        product_id.into_inner(),_tenant_id.inner()
    ).await?;
    Ok(HttpResponseBuilder::new(StatusCode::OK).json(ap))
}

setup_routes!(ProductItemService,"/product-item",
    "/create",web::post().to(create_product_item),
    "/id/{id}",web::get().to(get_product_item)
);
...
mod tests {
    ...
    async fn test_create_product_item() {
        ...
        get_and_create_api_test_v2!(ProductItemResponse,ProductItemService,closure,get_uri,
        "/product-item/create",request,response,*SEED_TENANT_ID);
