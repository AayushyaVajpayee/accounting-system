...
use crate::masters::company_master::company_unit_master::company_unit_models::{
    CompanyUnitAddressRequest, CompanyUnitMaster, CreateCompanyUnitRequest,
};
...
impl TryFrom<&Row> for CompanyUnitMaster {
    ...
    fn try_from(row: &Row) -> Result<Self, Self::Error> {
        ...
        Ok(CompanyUnitMaster {
            base_master_fields,
            company_id: row.try_get(next_ind)?,
            address_id: row.try_get(next_ind + 1)?,
            gstin: GstinNo::new(row.try_get(next_ind + 2)?)
                .context("error during db row conversion for gstin")?,
...
impl CompanyUnitDao for CompanyUnitDaoImpl {
    async fn create_company_unit(
        ...
        "#,
            ...
            CompanyUnitAddressRequest::NewAddress { .. } => {
                let addr_req = request1
                    .to_create_address_request()
    ...
    async fn get_company_unit_by_id(
        ...
        let cmp_unit: Option<CompanyUnitMaster> = self
            .postgres_client
            .get()
            .await?
            .query_opt(query, &[id])
    ...
    async fn get_company_units_by_company_id(
        ...
        let select_count_query = format!(
            "select count(*) from company_unit_master where company_id='{}'",
            company_id
        );
...
mod tests {
    ...
    async fn test_paginated_get_company_units_by_company_id() {
        ...
        let pg_pool = get_postgres_conn_pool(port, Some("get_company_units_by_company_id")).await;
        {
            pg_pool
                ...
                .await
                .unwrap()
                .simple_query("delete from company_unit_master")
                .await
                .unwrap();
        }
        let company_master_dao = CompanyUnitDaoImpl {
    ...
    async fn should_be_able_to_create_company_unit_with_existing_address_id(
        #[case] add_type: CompanyUnitAddressRequest,
    ) {
        let port = get_postgres_image_port().await;
        let postgres_client = get_postgres_conn_pool(port, None).await;
        let dao = CompanyUnitDaoImpl {
