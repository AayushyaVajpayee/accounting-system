...
pub async fn get_or_fetch_entity<T, Erro, Fut>(
    tenant_id: Uuid,
    entity_id: Uuid,
    cache: &Cache<(Uuid, Uuid), Arc<T>>,
    fetch: Fut,
...
mod tests {
    ...
    async fn test_cache_hit() {
        ...
        let _p = get_or_fetch_entity(tenant_id, id, &cache, async { mock.fetch().await }).await;
    }
    #[tokio::test]
    async fn test_cache_miss() {
        ...
        let cache: Cache<(Uuid, Uuid), Arc<E>> = Cache::new(1);
        let e = E {};
        mock.expect_fetch().once().return_once(|| Ok(Some(e)));
