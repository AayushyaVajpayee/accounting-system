...
async fn create_user(request: web::Json<CreateUserRequest>,
                     data: Data<Arc<dyn UserService>>,tenant_id: TenantId,user_id: UserId) -> actix_web::Result<impl Responder> {
    let p = data.create_user(&request.0,tenant_id.inner(),user_id.inner()).await?;
    Ok(web::Json(p))
}
...
mod tests {
    ...
    async fn test_api() {
        let closure= ||{
            ...
            mocked.expect_create_user().returning(|_,_,_| Ok(Default::default()));
            mocked
        };
