...
use crate::common_utils::utils::{
    get_current_indian_standard_time, get_current_time_us,
    parse_db_output_of_insert_create_and_return_uuid,
};
...
impl UserDao for UserDaoPostgresImpl {
    async fn get_user_by_id(&self, id: Uuid, tenant_id: Uuid) -> Result<Option<User>, DaoError> {
        let rows = self
            ...
            .get()
            .await?
            .query(BY_ID_QUERY, &[&id, &tenant_id])
            .await?;
        rows.iter().map(|row| row.try_into()).next().transpose()
    }
...
mod tests {
    ...
    use crate::accounting::user::user_models::tests::{
        a_create_user_request, CreateUserRequestTestBuilder,
    };
    use crate::tenant::tenant_models::tests::SEED_TENANT_ID;

    #[tokio::test]
    async fn should_be_able_to_create_and_fetch_users() {
        let user_dao = get_dao_generic(
            |a| UserDaoPostgresImpl {
                postgres_client: a.clone(),
            },
            None,
        )
        .await;
        let user = a_create_user_request(CreateUserRequestTestBuilder {
            tenant_id: Some(*SEED_TENANT_ID),
            ..Default::default()
        });
        let user_id = user_dao.create_user(&user, *SEED_USER_ID).await.unwrap();
        let user = user_dao
            .get_user_by_id(user_id, *SEED_TENANT_ID)
            .await
            .unwrap()
            .unwrap();
    ...
    async fn should_create_account_when_only_1_new_request() {
        ...
        let id = user_dao
            .create_user(&user_request, *SEED_USER_ID)
            .await
            .unwrap();
    ...
    async fn should_return_existing_account_when_idempotency_key_is_same_as_earlier_completed_request(
        ...
        .await;
        let name = "tsting";
        let user_request = a_create_user_request(CreateUserRequestTestBuilder {
        ...
        let number_of_users_created: i64 = user_dao
            .postgres_client
            .get()
            .await
            .unwrap()
