...
use crate::accounting::account::account_type::account_type_dao::{
    get_account_type_dao, AccountTypeDao,
};
...
pub trait AccountTypeService: Send + Sync {
    async fn get_account_type_hierarchy(
        &self,
        tenant_id: Uuid,
    ) -> Result<Vec<AccountTypeHierarchy>, AccountTypeServiceError>;
}
...
impl AccountTypeService for AccountTypeServiceImpl {
    async fn get_account_type_hierarchy(
        ...
        let all_accounts = self
            .dao
            .get_all_account_types_for_tenant_id(tenant_id)
            .await?;
...
pub fn get_account_type_master_service(arc: Arc<Pool>) -> Arc<dyn AccountTypeService> {
    ...
    let service = AccountTypeServiceImpl { dao };
    Arc::new(service)
}
...
impl AccountTypeServiceImpl {
    fn create_hierarchy(
        all_accounts: &[AccountTypeMaster],
    ) -> Result<Vec<AccountTypeHierarchy>, AccountTypeServiceError> {
        let account_map: HashMap<Uuid, &AccountTypeMaster> =
            all_accounts.iter().map(|r| (r.id, r)).collect();
        let root_accounts: Vec<&AccountTypeMaster> = all_accounts
            .iter()
        ...
        let root_accounts = if root_accounts.is_empty() {
            vec![all_accounts
                .iter()
                .min_by_key(|l| l.id)
                .ok_or(AccountTypeServiceError::EmptyChartOfAccounts)?]
        } else {
            root_accounts
        };
        root_accounts
    ...
    fn create_account_type_hierarchy(
        account_map: &HashMap<Uuid, &AccountTypeMaster>,
        root: &AccountTypeMaster,
    ) -> Result<AccountTypeHierarchy, AccountTypeServiceError> {
        let create_hierarchy_object = |id: &Uuid| -> AccountTypeHierarchy {
            AccountTypeHierarchy {
                current_account_id: id.clone(),
                child_account_types: account_map
                    .get(&id)
                    .unwrap()
                    .child_ids
                    .as_ref()
        ...
        while !queue.is_empty() && max_iter > 0 {
            ...
            for ah in children.iter_mut() {
                let master_item = account_map.get(&ah.current_account_id).ok_or(
                    AccountTypeServiceError::AccountIdNotPresentInChart(ah.current_account_id),
                )?;
                ah.child_account_types = master_item
                    .child_ids
                    .as_ref()
                    .map(|ids| {
                        ids.iter()
                    ...
                    .unwrap_or(vec![]);
                ah.child_account_types
                    .iter_mut()
                    .for_each(|t| queue.push(t))
            }
        }
        Ok(root_node)
    }
...
mod tests {
    ...
    use crate::accounting::account::account_type::account_type_models::AccountTypeMaster;
    use crate::accounting::account::account_type::account_type_service::{
        AccountTypeHierarchy, AccountTypeServiceImpl,
    };
    ...
    fn serialise_account_hierarchy(hierarchy: &mut AccountTypeHierarchy) -> String {
            ...
            if adj_entry.is_some() {
                ...
                adj.push(AdjacencyListEntry {
                    ...
                    adj_links: curr
                        .child_account_types
                        .iter()
                        .map(|l| l.current_account_id)
                        .collect::<HashSet<Uuid>>(),
                });
                curr.child_account_types
                    .iter()
                    .for_each(|a| work_queue.push(a));
            }
        }
        ...
        adj.iter()
            .map(|o| {
                ...
                new_str.push_str(o.id.to_string().as_str());
                new_str.push('[');
                let mut ooo = o
                    .adj_links
                    .iter()
    ...
    fn parse_account_tree(acc_tree: &str) -> Vec<AccountTypeMaster> {
        ...
        let pp = k
            ...
            .map(|k| {
                ...
                let id = p
                    .split_once('[')
                    .map(|l| l.0.parse::<Uuid>().expect(l.0))
                    .unwrap();
                let child_ids = p
                    .split_once('[')
                    .unwrap()
        ...
        pp.iter()
            .map(|o| create_account_type_master(o.id, &o.adj_links, find_parent_for_id(&pp, o.id)))
    ...
    fn test_create_account_type_hierarchy(
        #[case] account_tree: String,
        #[case] acc_id_range: String,
    ) {
        ...
        for n in range[0]..=range[1] {
            let timestmp = Timestamp::from_unix(
                NoContext,
                (get_current_time_us().unwrap() as u64) + (n as u64) * 1000,
                0,
            ); //to generate sortable uuids
            map.insert(
                n.to_string().chars().next().unwrap(),
                Uuid::new_v7(timestmp).to_string(),
            );
        }
