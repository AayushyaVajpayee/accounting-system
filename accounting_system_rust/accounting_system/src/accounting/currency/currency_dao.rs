...
use crate::common_utils::utils::{
    get_current_time_us, parse_db_output_of_insert_create_and_return_uuid, UserId,
};

const SELECT_FIELDS: &str =
    "id,entity_version_id,tenant_id,active,approval_status,remarks,scale,display_name,description,\
...
pub trait CurrencyDao: Send + Sync {
    async fn get_currency_entry_by_id(
        &self,
        id: Uuid,
        tenant_id: Uuid,
    ) -> Result<Option<CurrencyMaster>, DaoError>;
    async fn create_currency_entry(
        &self,
        currency: &CreateCurrencyMasterRequest,
        tenant_id: Uuid,
        user_id: Uuid,
...
impl CurrencyDao for CurrencyDaoPostgresImpl {
    async fn get_currency_entry_by_id(
        ...
        let k = conn.query(BY_ID_QUERY, &[&id, &tenant_id]).await?;
        let ans = k.iter().map(|row| row.try_into()).next().transpose()?;
        Ok(ans)
    }

    async fn create_currency_entry(
        &self,
        req: &CreateCurrencyMasterRequest,
        tenant_id: Uuid,
        user_id: Uuid,
    ) -> Result<Uuid, DaoError> {
...
mod tests {
    ...
    async fn should_be_able_to_create_and_fetch_currency() {
        let currency_dao = get_dao_generic(
            |a| CurrencyDaoPostgresImpl {
        ...
        let curr_id = currency_dao
            .create_currency_entry(&currency_master, *SEED_TENANT_ID, *SEED_USER_ID)
            .await
            .unwrap();
        let fetched_curr = currency_dao
            .get_currency_entry_by_id(curr_id, *SEED_TENANT_ID)
            .await
            .unwrap()
            .unwrap();
    ...
    async fn should_create_account_when_only_1_new_request() {
        ...
        let id = currency_dao
            .create_currency_entry(&currency_request, *SEED_TENANT_ID, *SEED_USER_ID)
            .await
            .unwrap();
        let curr = currency_dao
            .get_currency_entry_by_id(id, *SEED_TENANT_ID)
            .await
            .unwrap();
    ...
    async fn should_return_existing_account_when_idempotency_key_is_same_as_earlier_completed_request(
        ...
        let currency_request = a_create_currency_master_request(builder);
        let id = currency_dao
            .create_currency_entry(&currency_request, *SEED_TENANT_ID, *SEED_USER_ID)
            .await
            .unwrap();
        let id2 = currency_dao
            .create_currency_entry(&currency_request, *SEED_TENANT_ID, *SEED_USER_ID)
            .await
            .unwrap();
        assert_that!(&id).is_equal_to(id2);
        let number_of_currs_created: i64 = currency_dao
            .postgres_client
            .get()
            .await
            .unwrap()
