...
use crate::common_utils::utils::{get_current_time_us, parse_db_output_of_insert_create_and_return_uuid, UserId};
...
pub trait CurrencyDao: Send + Sync {
    ...
    async fn create_currency_entry(&self, currency: &CreateCurrencyMasterRequest,tenant_id:Uuid,user_id:Uuid) -> Result<Uuid, DaoError>;
}
...
impl CurrencyDao for CurrencyDaoPostgresImpl {
    ...
    async fn create_currency_entry(&self, req: &CreateCurrencyMasterRequest,tenant_id:Uuid,user_id: Uuid) -> Result<Uuid, DaoError> {
...
mod tests {
    ...
    async fn should_be_able_to_create_and_fetch_currency() {
        ...
        let curr_id = currency_dao.create_currency_entry(&currency_master,*SEED_TENANT_ID,*SEED_USER_ID).await.unwrap();
    ...
    async fn should_create_account_when_only_1_new_request() {
        ...
        let id = currency_dao.create_currency_entry(&currency_request,*SEED_TENANT_ID,*SEED_USER_ID).await.unwrap();
    ...
    async fn should_return_existing_account_when_idempotency_key_is_same_as_earlier_completed_request() {
        ...
        let id = currency_dao.create_currency_entry(&currency_request,*SEED_TENANT_ID,*SEED_USER_ID).await.unwrap();
        let id2 = currency_dao.create_currency_entry(&currency_request,*SEED_TENANT_ID,*SEED_USER_ID).await.unwrap();
