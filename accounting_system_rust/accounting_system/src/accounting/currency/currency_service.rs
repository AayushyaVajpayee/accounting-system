...
use crate::accounting::currency::currency_dao::{get_currency_dao, CurrencyDao};
...
pub trait CurrencyService: Send + Sync {
    async fn create_currency_entry(
        ...
        tenant_id: Uuid,
        user_id: Uuid,
    ) -> Result<Uuid, CurrencyServiceError>;
    async fn get_currency_entry(
        &self,
        id: Uuid,
        tenant_id: Uuid,
    ) -> Result<Option<Arc<CurrencyMaster>>, CurrencyServiceError>;
}
...
pub fn get_currency_service(arc: Arc<Pool>) -> Arc<dyn CurrencyService> {
    ...
    let cache: Cache<(Uuid, Uuid), Arc<CurrencyMaster>> = Cache::builder()
        ...
        .build();
    let currency_s = CurrencyServiceImpl {
        currency_dao,
        cache_by_tenant_id_and_currency_id: cache,
    };
    Arc::new(currency_s)
}
...
impl CurrencyService for CurrencyServiceImpl {
    async fn create_currency_entry(
        ...
        self.currency_dao
            .create_currency_entry(request, tenant_id, user_id)
            .await
            .map_err(|a| a.into())
    }
    async fn get_currency_entry(
        ...
        let fetch = async {
            let p = self
                .currency_dao
                .get_currency_entry_by_id(id, tenant_id)
                .await?;
            Ok(p)
        };
        get_or_fetch_entity(
            tenant_id,
            id,
            &self.cache_by_tenant_id_and_currency_id,
            fetch,
        )
        .await
    }
