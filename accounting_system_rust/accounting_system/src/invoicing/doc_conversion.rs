...
use pdf_doc_generator::invoice_template::InvoiceTableHeaderNameEnum::{
    Cgst, ItemDescription, LineTotal, Qty, SerialNo, Sgst, UnitPrice, Uqc,
};
use pdf_doc_generator::invoice_template::{
    AdditionalCharge, Address, DocDate, Invoice, InvoiceLineTable, InvoiceParty, InvoiceSummary,
    InvoiceTableHeaderNameEnum, TaxLine, TaxSummary,
};
...
use crate::invoicing::invoicing_request_models::{
    CreateInvoiceLineRequestWithAllDetails, CreateInvoiceWithAllDetailsIncluded,
};
...
pub async fn convert_to_invoice_doc_model<'a>(
    data_input: &'a InvoiceDocCreationDataInput<'a>,
    invoice_number: String,
    business_entity_service: Arc<dyn BusinessEntityService>,
    currency: Arc<CurrencyMaster>,
) -> anyhow::Result<Invoice> {
    let invoice = data_input.invoice;
    let supplier = fetch_business_entity(
        Some(invoice.supplier_id),
        invoice.tenant_id,
    ...
    let dispatcher = if invoice.supplier_id == invoice.dispatch_from_id {
        ...
        let entity = fetch_business_entity(
            Some(invoice.dispatch_from_id),
            invoice.tenant_id,
            business_entity_service.clone(),
        )
        .await?
    ...
    let billed_to = fetch_business_entity(
        invoice.billed_to_customer_id,
        invoice.tenant_id,
        business_entity_service.clone(),
    )
    .await?;
    let shipped_to = fetch_business_entity(
        invoice.shipped_to_customer_id,
        invoice.tenant_id,
        business_entity_service.clone(),
    )
    .await?;
...
fn create_invoice_line_table<'a>(
    data: &'a InvoiceDocCreationDataInput<'a>,
    currency: Arc<CurrencyMaster>,
) -> anyhow::Result<InvoiceLineTable> {
    let invoice = data.invoice;
    Ok(InvoiceLineTable {
        invoice_lines_total: invoice.invoice_lines.iter().map(|a| a.line_net_total).sum(),
        header_and_units: get_header_and_units(&invoice, currency),
        lines: invoice
            .invoice_lines
            .iter()
            .map(|a| convert_db_line_to_doc_line(a, invoice.igst_applicable))
...
fn create_invoice_tax_summary<'a>(
    data: &'a InvoiceDocCreationDataInput<'a>,
) -> anyhow::Result<TaxSummary> {
    let invoice = data.invoice;
...
impl InvoiceDb<'_> {
    ...
    fn batch_no_header(&self) -> Option<InvoiceTableHeaderNameEnum> {
        if self.invoice_lines.iter().all(|a| a.batch_no.is_none()) {
            None
        } else {
    ...
    fn mrp_header(&self, currency_unit: String) -> Option<InvoiceTableHeaderNameEnum> {
        if self.invoice_lines.iter().all(|a| a.mrp.is_none()) {
            None
        } else {
    ...
    fn cgst_sgst_header(
        &self,
        h: InvoiceTableHeaderNameEnum,
    ) -> Option<InvoiceTableHeaderNameEnum> {
    ...
    fn cess_header(&self) -> Option<InvoiceTableHeaderNameEnum> {
        if self.invoice_lines.iter().all(|l| l.cess_percentage == 0.0) {
            None
        } else {
...
fn get_header_and_units(
    invoice: &InvoiceDb,
    currency_master: Arc<CurrencyMaster>,
) -> Vec<InvoiceTableHeaderNameEnum> {
...
fn convert_db_line_to_doc_line(
    a: &InvoiceLineDb,
    igst_applicable: bool,
) -> anyhow::Result<pdf_doc_generator::invoice_template::InvoiceLine> {
    let line = pdf_doc_generator::invoice_template::InvoiceLine {
        ...
        igst_percentage: if igst_applicable {
            a.tax_percentage
        } else {
            0.0
        },
        cgst_percentage: if igst_applicable {
            0.0
        } else {
            a.tax_percentage / 2.0
        },
        sgst_percentage: if igst_applicable {
            0.0
        } else {
            a.tax_percentage / 2.0
        },
...
async fn fetch_business_entity(
    id: Option<Uuid>,
    tenant_id: Uuid,
    service: Arc<dyn BusinessEntityService>,
) -> anyhow::Result<Option<Arc<BusinessEntityDto>>> {
    if let Some(id) = id {
...
fn convert_to_tax_lines(
    data: &CreateInvoiceWithAllDetailsIncluded,
    igst_applicable: bool,
) -> anyhow::Result<Vec<TaxLine>> {
    let mut lines_and_tax_amt: Vec<(&CreateInvoiceLineRequestWithAllDetails, f64)> =
        Vec::with_capacity(data.invoice_lines.len());
    ...
    for line in lines_and_tax_amt.iter() {
        ...
        let tax_bps = (req
            ...
            * 100.0)
            .round() as u32;
        grouped_tax_bps_with_tax_amt_lines
            .entry(tax_bps)
