...
use invoice_doc_generator::percentages::tax_discount_cess::{
    CessPercentage, DiscountPercentage, GSTPercentage,
};
...
pub(crate) struct Invoice {
    ...
    dispatch_from_business_entity_id: Uuid,
    b2b_invoice: bool,
...
pub mod tests {
    ...
    use crate::invoicing::invoicing_domain_models::{
        AdditionalCharge, AdditionalChargeBuilder, Invoice, InvoiceBuilder, InvoiceLine,
        InvoiceLineBuilder,
    };
    ...
    use crate::masters::business_entity_master::business_entity_models::tests::{
        SEED_BUSINESS_ENTITY_ID2, SEED_BUSINESS_ENTITY_INVOICE_DTL_ID1,
    };
    ...
    pub fn an_invoice_line(builder: InvoiceLineBuilder) -> InvoiceLine {
        InvoiceLine {
            ...
            line_title_hsn_sac_id: builder
                .line_title_hsn_sac_id
                .unwrap_or(*SEED_LINE_TITLE_HSN_ID),
            line_subtitle_id: builder.line_subtitle_id.unwrap_or(Some(*SEED_SUBTITLE_ID)),
            quantity: builder
                .quantity
                .unwrap_or(a_line_quantity(Default::default())),
            unit_price: builder.unit_price.unwrap_or(Price::new(10.0).unwrap()),
            tax_rate_bps: builder
                .tax_rate_bps
                .unwrap_or(GSTPercentage::new(28.0).unwrap()),
            discount_percentage: builder
                .discount_percentage
                .unwrap_or(DiscountPercentage::new(0.0).unwrap()),
            cess_percentage: builder
                .cess_percentage
                .unwrap_or(CessPercentage::new(0.0).unwrap()),
    ...
    pub(crate) fn an_invoice(builder: InvoiceBuilder) -> Invoice {
        Invoice {
            ...
            financial_year: builder
                .financial_year
                .unwrap_or_else(|| current_indian_financial_year() as i16),
            invoice_number: builder
                .invoice_number
                .unwrap_or_else(|| InvoiceNumber::new("ABC123".to_string()).unwrap()),
            currency_id: builder.currency_id.unwrap_or(*SEED_CURRENCY_ID),
            service_invoice: builder.service_invoice.unwrap_or(false),
            invoice_date_ms: builder
                .invoice_date_ms
                .unwrap_or_else(|| get_current_time_us().unwrap() / 1000),
            e_invoicing_applicable: builder.e_invoicing_applicable.unwrap_or(false),
            supplier_business_entity_id: builder
                .supplier_business_entity_id
                .unwrap_or(*SEED_BUSINESS_ENTITY_INVOICE_DTL_ID1),
            dispatch_from_business_entity_id: builder
                .dispatch_from_business_entity_id
                .unwrap_or(*SEED_BUSINESS_ENTITY_INVOICE_DTL_ID1),
            b2b_invoice: builder.b2b_invoice.unwrap_or(false),
            billed_to_business_entity_id: builder
                .billed_to_business_entity_id
                .unwrap_or(Some(*SEED_BUSINESS_ENTITY_ID2)),
            shipped_to_business_entity_id: builder
                .shipped_to_business_entity_id
                .unwrap_or(Some(*SEED_BUSINESS_ENTITY_ID2)),
