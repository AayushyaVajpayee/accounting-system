...
pub trait InvoicingSeriesDao: Send + Sync {
    ...
    async fn get_invoicing_series_by_id(&self, invoicing_series_id: &Uuid) -> Result<Option<InvoicingSeriesMaster>, DaoError>;
}
...
impl InvoicingSeriesDao for InvoicingSeriesDaoImpl {
    ...
    async fn get_invoicing_series_by_id(&self, invoicing_series_id: &Uuid) -> Result<Option<InvoicingSeriesMaster>, DaoError> {
        ...
        let jj = self.postgres_client.get().await?
            .query_opt(query, &[&invoicing_series_id]).await?
            .map(|a| a.try_into())
            .transpose()?;
        Ok(jj)
    }
...
mod tests {
    ...
    async fn test_insert_invoicing_series() {
        ...
        // let postgres_client = get_postgres_conn_pool(5435, None).await;
        ...
        // let jj = dao.get_invoicing_series_by_id(&p).await.unwrap();
