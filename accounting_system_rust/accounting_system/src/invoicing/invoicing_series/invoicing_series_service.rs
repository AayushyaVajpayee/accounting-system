...
use crate::invoicing::invoicing_series::invoicing_series_dao::{
    get_invoicing_series_dao, InvoicingSeriesDao,
};
use crate::invoicing::invoicing_series::invoicing_series_models::{
    CreateInvoiceNumberSeriesRequest, InvoicingSeriesMaster,
};
...
pub trait InvoicingSeriesService: Send + Sync {
    async fn create_invoice_series(
        ...
        tenant_id: Uuid,
        user_id: Uuid,
    ) -> Result<Uuid, InvoicingSeriesServiceError>;

    async fn get_invoicing_series_by_id(
        &self,
        invoicing_series_id: Uuid,
        tenant_id: Uuid,
    ) -> Result<Option<Arc<InvoicingSeriesMaster>>, InvoicingSeriesServiceError>;
    async fn is_valid_invoicing_series_id(
        &self,
        invoicing_series_id: Uuid,
        tenant_id: Uuid,
    ) -> Result<bool, InvoicingSeriesServiceError>;
}
...
impl InvoicingSeriesService for InvoicingSeriesServiceImpl {
    async fn create_invoice_series(
        ...
        let d = self
            .dao
            .create_invoice_series(&request, tenant_id, user_id)
            .await?;
        Ok(d)
    }
