...
mod tests {
    ...
    fn convert_to_db_add_charge_input(
        charges: &Vec<CreateAdditionalChargeRequestDbModel>,
    ) -> String {
        let mut k: Vec<String> = Vec::new();
        for x in charges {
            let p = format!(
                "('{}',{}::smallint,'{}',{},{})",
                x.line_id, x.line_no, x.line_title, x.title_xx_hash, x.rate
            );
            k.push(p);
        }
    ...
    async fn test() {
        ...
        commit;
        "#,
            convert_to_db_add_charge_input(&charges),
            *SEED_INVOICE_ID,
            *SEED_TENANT_ID,
            *SEED_USER_ID
        );
        ...
        let asd = postgres_client
            .get()
            .await
            .unwrap()
            .query_opt("select * from additional_charge where id =$1", &[&id])
            .await
            .unwrap();
