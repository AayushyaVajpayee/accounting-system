...
pub trait LedgerTransferDao: Send + Sync {
    ...
    async fn create_batch_transfers(
        &self,
        transfers: &[Vec<Transfer>],
    ) -> Vec<Vec<TransferCreationDbResponse>>;
...
impl TryFrom<&Row> for Transfer {
    ...
    fn try_from(row: &Row) -> Result<Self, Self::Error> {
        ...
        let transfer_type = match transfer_type_numeric_code {
            ...
            _ => panic!(
                "{} is not mapped to transferType enum",
                transfer_type_numeric_code
            ), //todo error handling  part. also will have to write test for this too
        };
        Ok(Transfer {
            id: row.get(0),
            tenant_id: row.get(1),
    ...
    fn get_transfer_by_id_query() -> &'static str {
        TRANSFER_BY_ID_QUERY.get_or_init(|| {
            format!(
                ...
                LEDGER_TRANSFER_POSTGRES_SELECT_FIELDS, LEDGER_TRANSFER_TABLE_NAME
            )
        })
    }
}
...
pub fn get_ledger_transfer_dao(pool: Arc<Pool>) -> Arc<dyn LedgerTransferDao> {
    Arc::new(LedgerTransferDaoPostgresImpl {
...
impl LedgerTransferDao for LedgerTransferDaoPostgresImpl {
    async fn create_transfers(&self, transfers: &[Transfer]) -> Vec<TransferCreationDbResponse> {
        ...
        let transfers_db_response = match query_results {
            ...
            SimpleQueryMessage::CommandComplete(_) => {
                todo!()
            } //todo this will come in error handling part. will have to write test for this too. writing test may not be possible for this
            _ => {
                todo!()
            } //todo this will come in error handling part. will have to write test for this too. writing test may not be possible for this
        };
    ...
    async fn get_transfers_by_id(&self, id: Uuid) -> Option<Transfer> {
        ...
        let conn = self.postgres_client.get().await.unwrap();
        let k = conn.query(query, &[&id]).await.unwrap();
...
fn convert_transfer_to_postgres_composite_type_input_string(transfer: &Transfer) -> String {
    format!(
        ...
        match transfer.transfer_type {
            ...
            PostPending { pending_id } | VoidPending { pending_id } => {
                format!("'{}'", pending_id)
            }
        },
...
mod tests {
    ...
    use crate::accounting::account::account_models::tests::{
        a_create_account_request, CreateAccountRequestTestBuilder, SEED_CREDIT_ACCOUNT_ID,
        SEED_DEBIT_ACCOUNT_ID,
    };
    ...
    use crate::ledger::ledgermaster::ledger_master_models::tests::{
        a_create_ledger_master_entry_request, SEED_LEDGER_MASTER_ID,
    };
    ...
    fn generate_random_transfers(
        debit_account: Uuid,
        credit_account: Uuid,
        amount: i64,
        ledger_master_id: Uuid,
        size: usize,
    ) -> Vec<Transfer> {
    ...
    mod create_batch_transfer_tests {
        ...
        use crate::ledger::ledger_transfer_dao::tests::{
            create_two_accounts_for_transfer, generate_random_transfers,
        };
        use crate::ledger::ledger_transfer_dao::{
            LedgerTransferDao, LedgerTransferDaoPostgresImpl,
        };
        ...
        async fn should_be_able_to_post_multiple_linked_transfers(
            ...
            let ledger_transfer_dao = get_dao_generic(
                |a| LedgerTransferDaoPostgresImpl {
            ...
            let transfer = generate_random_transfers(
                accs[0],
                accs[1],
                100,
            ...
            .chunks(inner_arr_size as usize)
            .map(|a| a.to_vec())
            .collect::<Vec<Vec<Transfer>>>();
            let batch_transfer_responses =
                ledger_transfer_dao.create_batch_transfers(&transfer).await;
        ...
        async fn should_fail_only_one_batch_due_to_error_and_not_others() {
            ...
            let accs = create_two_accounts_for_transfer().await;
            let mut a_trf =
                generate_random_transfers(accs[0], accs[1], 100, *SEED_LEDGER_MASTER_ID, 2)
                    .chunks(2_usize)
                    .map(|a| a.to_vec())
                    .collect::<Vec<Vec<Transfer>>>();
            ...
            for trf_resp in &batch_trfs_resp[0] {
                assert!(trf_resp.committed);
                assert!(trf_resp.reason.is_empty());
                assert!(ledger_transfer_dao
                    .get_transfers_by_id(trf_resp.txn_id)
                    .await
                    .is_some())
            }
        ...
        async fn should_panic_if_transfer_more_than_500() {
            ...
            let accs = create_two_accounts_for_transfer().await;
            let mut a_trf =
                generate_random_transfers(accs[0], accs[1], 100, *SEED_LEDGER_MASTER_ID, 502)
                    .chunks(50usize)
                    .map(|a| a.to_vec())
                    .collect::<Vec<Vec<Transfer>>>();
            let sample_trfs = generate_random_transfers(
    ...
    mod transfer_type_pending_tests {
        ...
        async fn test_posting_a_pending_entry(#[case] entry_type: Option<TransferType>) {
            let led_trf_dao = get_dao_generic(
                |a| LedgerTransferDaoPostgresImpl {
                    postgres_client: a.clone(),
                },
                None,
            ...
            .await;
            let accs = create_two_accounts_for_transfer().await;
            let acc_ser = get_account_service_for_test(led_trf_dao.postgres_client.clone());
    ...
    mod pending_transfer_resolution_tests {
        ...
        async fn should_be_able_to_resolve_pending_transfer(
            ...
            #[case] pending_resolution_type: Option<TransferType>,
            #[case] pending_resolution_amount: Option<i64>,
        ) {
            let pending_resolution_type = match pending_resolution_type.unwrap() {
                TransferType::Regular | TransferType::Pending => {
                    panic!("invalid state")
                }
                TransferType::PostPending { .. } => TransferType::PostPending {
                    pending_id: pending_transfer.id,
                },
                TransferType::VoidPending { .. } => TransferType::VoidPending {
                    pending_id: pending_transfer.id,
                },
            };
            let cl = get_dao_generic(
                |a| LedgerTransferDaoPostgresImpl {
                    postgres_client: a.clone(),
                },
                None,
            ...
            let acc_ser = get_account_service_for_test(cl.postgres_client.clone());
            let acc1 = acc_ser
                .get_account_by_id(&resolved_pending_transfer.debit_account_id)
                .await
                .unwrap()
                .unwrap();
            let acc2 = acc_ser
                .get_account_by_id(&resolved_pending_transfer.credit_account_id)
                .await
                .unwrap()
                .unwrap();
            let trf_resps = cl
                .create_transfers(&vec![pending_transfer, resolved_pending_transfer.clone()])
                .await;
            println!("{:?}", trf_resps);
            let acc1_after = acc_ser
                .get_account_by_id(&resolved_pending_transfer.debit_account_id)
                .await
                .unwrap()
                .unwrap();
            let acc2_after = acc_ser
                .get_account_by_id(&resolved_pending_transfer.credit_account_id)
                .await
                .unwrap()
                .unwrap();
            if matches!(
                resolved_pending_transfer.transfer_type,
                TransferType::PostPending { .. }
            ) {
                assert_eq!(
                    acc1.debits_posted + resolved_pending_transfer.amount,
                    acc1_after.debits_posted
                );
                ...
                assert_eq!(
                    acc2.credits_posted + resolved_pending_transfer.amount,
                    acc2_after.credits_posted
                );
            }
            if matches!(
                resolved_pending_transfer.transfer_type,
                TransferType::VoidPending { .. }
            ) {
            ...
            let _fetched_trf_resp = cl
                .get_transfers_by_id(resolved_pending_transfer.id)
                .await
                .unwrap();
        ...
        #[trace]
        async fn should_error_out_posting_entry_for_an_invalid_pending_entry_id(
            #[case] transfer_type: Option<TransferType>,
        ) {
            let led_trf_dao = get_dao_generic(
                |a| LedgerTransferDaoPostgresImpl {
                    postgres_client: a.clone(),
            ...
            .await;
            let accs = create_two_accounts_for_transfer().await;
            let acc_service = get_account_service_for_test(led_trf_dao.postgres_client.clone());
            let acc1 = acc_service
                .get_account_by_id(&accs[0])
                .await
                .unwrap()
                .unwrap();
            let acc2 = acc_service
                .get_account_by_id(&accs[1])
                .await
                .unwrap()
                .unwrap();
            ...
            let trf_resps = led_trf_dao
                .create_transfers(&vec![pending_trf.clone(), resolved_pending_trf.clone()])
                .await;
            println!("{:?}", &trf_resps);
            let acc1_after = acc_service
                .get_account_by_id(&accs[0])
                .await
                .unwrap()
                .unwrap();
            let acc2_after = acc_service
                .get_account_by_id(&accs[1])
                .await
                .unwrap()
                .unwrap();
            ...
            assert_eq!(acc2.credits_posted, acc2_after.credits_posted);
            assert!(led_trf_dao
                .get_transfers_by_id(pending_trf.id)
                .await
                .is_none());
            assert!(led_trf_dao
                .get_transfers_by_id(resolved_pending_trf.id)
                .await
                .is_none());
            assert_eq!(2, trf_resps.len());
        ...
        async fn should_not_act_on_an_already_resolved_pending_transfer(
            #[values("pp", "vp")] first_resolved_transfer_type: String,
            #[values("pp", "vp")] second_resolved_transfer_type: String,
        ) {
            ...
            let acc2 = acc_service
                ...
                .unwrap()
                .unwrap();
            let led_trf_dao = LedgerTransferDaoPostgresImpl {
                postgres_client: led_trf_dao.postgres_client.clone(),
            };
            let pending_trf = a_transfer(TransferBuilder {
                transfer_type: Some(TransferType::Pending),
                debit_account_id: Some(accs[0]),
            ...
            let first_resolved_trf = a_transfer(TransferBuilder {
                transfer_type: if first_resolved_transfer_type == "pp" {
                    Some(TransferType::PostPending {
                        pending_id: pending_trf.id,
                    })
                } else {
                    Some(TransferType::VoidPending {
                        pending_id: pending_trf.id,
                    })
                },
                debit_account_id: Some(accs[0]),
            ...
            let trf_resps = led_trf_dao
                .create_transfers(&vec![
                    pending_trf.clone(),
                    first_resolved_trf.clone(),
                    second_resolved_trf.clone(),
                ])
                .await;
            let acc1_after = acc_service
                .get_account_by_id(&accs[0])
            ...
            assert!(led_trf_dao
                ...
                .is_none());
            assert!(led_trf_dao
                .get_transfers_by_id(first_resolved_trf.id)
                .await
                .is_none());
            assert!(led_trf_dao
                .get_transfers_by_id(second_resolved_trf.id)
                .await
                .is_none());
        ...
        async fn should_not_post_pending_entry_for_a_pending_entry_in_excess() {
            ...
            let post_pending_trf = a_transfer(TransferBuilder {
                transfer_type: Some(TransferType::PostPending {
                    pending_id: pending_trf.id.clone(),
                }),
                debit_account_id: Some(accs[0]),
                credit_account_id: Some(accs[1]),
                amount: Some(101),
            ...
            let trf_resps = led_trf_dao
                .create_transfers(&vec![pending_trf.clone(), post_pending_trf.clone()])
                .await;
            println!("{:?}", &trf_resps);
            let acc1_after = acc_service
                .get_account_by_id(&accs[0])
            ...
            assert!(led_trf_dao
                ...
                .is_none());
            assert!(led_trf_dao
                .get_transfers_by_id(post_pending_trf.id)
                .await
                .is_none());
            assert_eq!(2, trf_resps.len());
            for trf_resp in trf_resps {
        ...
        #[rstest]
        async fn should_not_resolve_a_post_transfer(#[values("pp")] resolution_type: String) {
            //todo important test
            let led_trf_dao = get_dao_generic(
                |a| LedgerTransferDaoPostgresImpl {
                    postgres_client: a.clone(),
            ...
            let resolving_trf = a_transfer(TransferBuilder {
                transfer_type: if resolution_type == "pp" {
                    Some(TransferType::PostPending {
                        pending_id: regular_trf.id.clone(),
                    })
                } else {
                    Some(TransferType::VoidPending {
                        pending_id: regular_trf.id.clone(),
            ...
            //todo first 2 should be committed together and third one should fail.
            let trf_resps = led_trf_dao
                .create_transfers(&vec![regular_trf.clone(), resolving_trf.clone()])
                .await;
            println!("{:?}", &trf_resps);
            let acc1_after = acc_service
                .get_account_by_id(&accs[0])
            ...
            assert_eq!(acc2.credits_posted, acc2_after.credits_posted);
            assert!(led_trf_dao
                .get_transfers_by_id(regular_trf.id)
                .await
                .is_none());
            assert!(led_trf_dao
                .get_transfers_by_id(resolving_trf.id)
                .await
                .is_none());
            assert_eq!(2, trf_resps.len());
            for trf_resp in trf_resps {
    ...
    async fn should_not_commit_transactions_which_have_been_already_persisted_idempotency() {
        ...
        let accs = create_two_accounts_for_transfer().await;
        let initial_trfs =
            generate_random_transfers(accs[0], accs[1], 100, *SEED_LEDGER_MASTER_ID, 1);
        ...
        assert_eq!(
            trf_resps_2.first().unwrap().reason[0],
            "transfer already exists with this id"
        );
        let mut more_trfs =
            generate_random_transfers(accs[0], accs[1], 100, *SEED_LEDGER_MASTER_ID, 2);
        ...
        for trf_resp in trf_resps_3 {
            ...
            assert!(
                trf_resp.reason[0] == "transfer already exists with this id"
                    || trf_resp.reason[0] == "linked transfer failed"
            )
        }
    }
    ...
    async fn should_fail_for_more_than_600_in_batch() {
        ...
        let transfer_candidates =
            generate_random_transfers(accs[0], accs[1], 100, *SEED_LEDGER_MASTER_ID, 601);
    ...
    async fn test_successful_create_transfers_of_multiple_sizes(#[case] size: usize) {
        ...
        let acc2 = acc_service
            ...
            .unwrap();
        let transfer_candidates =
            generate_random_transfers(accs[0], accs[1], 100, *SEED_LEDGER_MASTER_ID, size);
    ...
    async fn should_fail_if_accounts_have_different_ledger_ids(
        #[case] debit_account_ledger_id_same: bool,
        #[case] credit_account_ledger_same: bool,
        #[case] transfer_ledger_id_same: bool,
    ) {
        let led_trf_dao = get_dao_generic(
            |a| LedgerTransferDaoPostgresImpl {
                postgres_client: a.clone(),
        ...
        .await;
        let ledger_master_service =
            get_ledger_master_service_for_test(led_trf_dao.postgres_client.clone());
        let led_mst_req = a_create_ledger_master_entry_request(Default::default());
        let account_master_service =
            get_account_service_for_test(led_trf_dao.postgres_client.clone());
        ...
        if !debit_account_ledger_id_same {
            db_acc_led_id = ledger_master_service
                .create_ledger_master_entry(&led_mst_req)
                .await;
            db_acc_id = account_master_service
                .create_account(&a_create_account_request(CreateAccountRequestTestBuilder {
        ...
        if !credit_account_ledger_same {
            cr_acc_led_id = ledger_master_service
                .create_ledger_master_entry(&led_mst_req)
                .await;
            cr_acc_id = account_master_service
                .create_account(&a_create_account_request(CreateAccountRequestTestBuilder {
        ...
        if transfer_ledger_id_same {
            tr_led_id = ledger_master_service
                .create_ledger_master_entry(&led_mst_req)
                .await;
        }
        let transfer_candidates =
            generate_random_transfers(db_acc_id, cr_acc_id, 100, tr_led_id, 1);
        let trf_resps = led_trf_dao.create_transfers(&transfer_candidates).await;
        ...
        assert_eq!(
            trf_resps.first().unwrap().reason.first().unwrap(),
            err_message.as_str()
        );
    }
    ...
    async fn should_fail_transfer_amounts_of_less_than_equal_to_zero(#[case] amount: i64) {
        ...
        let acc2 = acc_service
            ...
            .unwrap();
        let transfer_candidates =
            generate_random_transfers(accs[0], accs[1], amount, *SEED_LEDGER_MASTER_ID, 1);
        let trf_resps = led_trf_dao.create_transfers(&transfer_candidates).await;
        let acc1_after = acc_service
            .get_account_by_id(&accs[0])
            .await
        ...
        assert_eq!(trf_resps.first().unwrap().reason.len(), 1);
        assert_eq!(
            trf_resps.first().unwrap().reason[0],
            format!("transfer amount cannot be <=0 but was {}", amount).as_str()
        );
    }
}
