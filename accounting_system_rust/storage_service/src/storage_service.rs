...
impl Storage for AwsStorageService {
    async fn upload_object(
        ...
        let _ = self
            ...
            .send()
            .await
            .context("error during object upload")?;
        let uri = self
    ...
    async fn get_object_url(
        ...
        let po = self
            ...
            .key(asset_name)
            .presigned(
                PresigningConfig::expires_in(expiry_time)
...
mod tests {
    ...
    async fn test_upload_fetch_and_deletion_of_object() {
        ...
        let p = reqwest::get(r).await.unwrap().bytes().await.unwrap();

        let s = String::from_utf8(p.to_vec()).unwrap();
        assert_that!(s).is_equal_to(text.to_string());
        let deletion_result = storage_service
            .delete_object(UNIT_TESTS_BUCKET, &asset_name)
            .await;
