...
async fn create_invoice_series(data: Data<Arc<dyn InvoicingSeriesService>>, request: web::Json<CreateInvoiceNumberSeriesRequest>) -> actix_web::Result<impl Responder> {
    let ap = data.create_invoice_series(&request).await?;
    Ok(HttpResponseBuilder::new(StatusCode::OK).json(ap))
}

async fn get_invoice_series(data: Data<Arc<dyn InvoicingSeriesService>>, invoicing_series_id: Path<Uuid>) -> actix_web::Result<impl Responder> {
    let ap = data.get_invoicing_series_by_id(&invoicing_series_id).await?;
    Ok(HttpResponseBuilder::new(StatusCode::OK).json(ap))
}
